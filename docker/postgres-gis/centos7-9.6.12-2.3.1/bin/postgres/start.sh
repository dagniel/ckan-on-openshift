...
########## Customized starting from original: https://github.com/CrunchyData/crunchy-containers/blob/2.3.1/bin/postgres/start.sh
# initidb phase for extra env vars and steps in setup.sql
# run under temporary user/group with nss_wrapper ??

#source /opt/cpm/bin/generate_container_user
...
function initialize_primary() {
    ...
    if [ ! -f ${PGDATA?}/postgresql.conf ]; then
        ...
        # extra environment for CKAN initialization
        sed -i "s/PG_DATASTORE_DB/$PG_DATASTORE_DB/g" /tmp/setup.sql
        sed -i "s/PG_DATASTORE_RO_USER/$PG_DATASTORE_RO_USER/g" /tmp/setup.sql
        sed -i "s/PG_DATASTORE_RO_PASS/$PG_DATASTORE_RO_PASS/g" /tmp/setup.sql
